        ...
        if (!stale && _transformCache.text) {
          ...
          if (microTools.length > 0) {
            const top = [...microTools]
              ...
              .slice(0, 5)
              .map(t => `- **${t.name}** (used ${t.uses}x): ${t.trigger}${t.autoGenerated ? ' (auto)' : ''}`);
            parts.push(`## Your Micro-Tools\n${top.join('\n')}\n→ If relevant: call \`evolve_lookup(situation)\` to get steps, then \`evolve_use\` after completion.`);
          }

          if (rules.length > 0) {
            const highRules = rules.filter(r => r.priority === 'high');
            if (highRules.length) {
              parts.push(`## Active Rules (high priority)\n${highRules.map(r => `- WHEN "${r.when}" → ${r.then} (fired ${r.fired}x)`).join('\n')}\n→ At task start: if any rule matches, call \`evolve_check_rules(situation)\` to fire it.`);
            }
          }
