...
├── .opencode/
│   ├── agent/                ← 9 agent definitions (zara.md + 8 specialists)
...
│   ├── plugin/               ← Plugin modules (8 domains)
...
The MCP server (8 domains, 30+ tools) runs locally via stdio:
...
# Check agent files
ls .opencode/agent/*.md | wc -l    # Should show 9
